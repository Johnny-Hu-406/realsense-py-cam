...
def save_frame(depth_frame, color_frame,file_name , depth_colormap, color_image):
    ...
    cv2.imwrite(file_name+"depth.png", depth_colormap)
    cv2.imwrite(file_name+"color.png", color_image)
...
try:
    while True:
        ...
        # Show images
        image_scaled = cv2.resize(images, (1280, 360), interpolation=cv2.INTER_AREA)
        cv2.namedWindow('RealSense', cv2.WINDOW_AUTOSIZE)
        cv2.imshow('RealSense', image_scaled)
        ...
        if save_flag:
            ...
            # 保存图像
            save_frame(depth_frame.get_data(), color_frame.get_data(),file_name , depth_colormap, color_image)
